...
1. WX uses WXML (Wohola X Markup Language or Wohola XML) an enhanced version of HTML/CSS so developers with HTML/CSS can do most of the project developement. There is no need for developers to have skills in PHP, Python, Node.js or other such technologies.
...
14. WX is a webserver in itself, so there is no need for IIS, Apache or Nginx web server for running WX solutions. However, if those webservers are there WX can work with those servers acting as proxy.
...
---------------------
1. WX scales automatically based on server resources. It can also be configured to run on server farms for large deployments. The primary choice of server is Windows as per WX policy of ease of use so smaller companies can manage the WX deployment by training normal Windows users to manage the deployment. On the other hand larger enterprises who have skilled resources available for managing Linux deployment can opt for Linux option.
...
11. WX runs on your own servers and the data is stored on your own servers.
12. Any updates to WX would be published here regularly and these can be downloaded and deployed on your servers freely. It is as simple as copying the folder.

We welcome technical evaluation and benchmarking with your typical deployment for you to be able to compare the difference. Seeing is believing, so you can contact our solution providers for a demo or post a message here.

Notes for Developers of Web Based Solutions
-------------------------------------------
1. WX comes with a database structure which is suitable for 90% of typical business requirements. It creates and manages the database as well as data internally with zero coding. So you only create layout of what the screen looks like and WX figures out where and how to store and retrieve the data. The standard database can be enhanced, customized or replaced altogether on need basis.
2. Since HTML/XML is very basic language that practically does nothing, for developers working on complex technologies like PHP, React, Java or Angular and it is difficult to imagine being able to build full scale solutions using WXML, a language that is simply an enhanced version of HTML. You can read through quick start guide quickly and if you have time try the examples in the demo to see that its actually possible.
...
4. With open source technologies or with solutions built on PHP, Python or Javascript it can be difficult to protect your source code. In WX it is possible for you to deploy an encrypted version of XTENS and/or JSON files on client server where they can run the application but cannot view or modify your source code.
5. You can develop and deploy applications quicker and build new solutions easily and have less problems overall. Users would also like the ease of use of final outcome.
...
Limitations
-----------
1. Currently WX can only cater to Web Based requirements. It cannot take care of Mobile apps for the time being. The roadmap includes WXML to hybrid as well as native app translation. So WXML could be used for app development as well.
...
3. WX is based on licensing model. You can download and run wx.exe and the xtens freely for learning, development, testing and other such purposes using localhost. This exe would have certain issues if you make any sites live on a real domain without a license file. You can send us a message to find out more about the license file. Meanwhile you can try it out with the quick start guide in docs folder and with the demo code within this repository.
...
Please feel free to contact us or one of our solution providers for further information, see WX in action, join our free training programs, find out about licensing or to discuss your project requirements.
